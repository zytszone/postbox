...
public class TakeGoodsServiceImpl implements TakeGoodsService {
    ...
    private static final BigDecimal BIDCECIMAL_ZERO = new BigDecimal(0);
    private static final BigDecimal BIDCECIMAL_ONE = new BigDecimal(1);
    ...
    public PayInfo produceOrder(DeliveryOrder deliveryOrder) throws IOException {
        ...
        if(amount<=0){
            //不需要调用微信支付接口，直接完成订单
//            this.updateTakeGoodsNoPay(deliveryOrder);//不需要支付时直接完成订单(填完物流单号后执行)
            this.updateDeliveryInfoStatusByPaid(deliveryOrder);
            return null;
        }
        PayInfo payOrder = payService.createPayOrder(deliveryOrder.getUserId(), 1L, amount, PayBizType.DELIVERY, deliveryOrder.getDeliveryOrderId().toString(), null, "test");
    ...
    @Transactional
    @Override
    public void paidPickUp(Long payInfoId) throws WxException {
    ...
    public void expressDeliveryinfo(Long deliveryOrderId,String logisticsOrderId,String logisticsOperator) throws IOException {
        ...
        if(new BigDecimal(0).compareTo(deliveryOrder.getAddOnCommodityUnitQuantity()) ==-1){
            ...
            this.tradeAccountService.baseUnfreeze(deliveryOrder.getCommodityContractId(),operatorInfo.getOperatorInfoId(),AccountType.USER,0L,AccountType.OPERATOR,deliveryOrder.getAddOnCommodityUnitQuantity(),CommodityFlowType.PICK_UP,"提货申请完成","提货申请完成");
        }
    ...
    @Transactional
    @Override
    public RespResult payForDetailTakeGoods(Long bizId, UserLoginInfo userLoginInfo, HttpServletRequest request) throws IOException {
        ...
        wxPayParams.put("body", "掌上赢家-微信支付测试");
    ...
    @Transactional
    @Override
    public void cancelTakeGoods(Long deliveryOrderId, Long userInfoId) throws IOException {
        DeliveryOrder deliveryOrder = this.deliveryOrderMapperExt.findAndLockDeliveryOrderByDeliveryOrderId(deliveryOrderId);
     ...
     */
    @Override
    public void cancelExpireTakeGoods(Long deliveryOrderId) throws IOException {
        DeliveryOrder deliveryOrder = this.deliveryOrderMapperExt.findAndLockDeliveryOrderByDeliveryOrderId(deliveryOrderId);
    ...
    /**
     * 提货时不需要付钱时，直接更新所有相关订单信息
     *
     * @param deliveryOrder
     */
    @Transactional
    @Override
    public void updateTakeGoodsNoPay(DeliveryOrder deliveryOrder) {
        //调用解冻接口
        this.tradeAccountService.baseUnfreeze(deliveryOrder.getCommodityContractId(),deliveryOrder.getUserId(),AccountType.USER,0L,AccountType.OPERATOR,deliveryOrder.getActConsumeCommUnitQuantity(),CommodityFlowType.PICK_UP,"提货申请完成","提货申请完成");
    ...
    private boolean cluOrder(GiftAccount giftAccount,DeliveryOrder deliveryOrder) throws IOException {
        try {
            ...
            deliveryOrder.setCouponUnitQuantity(BIDCECIMAL_ZERO);//目前没有添加优惠券
            ...
            deliveryOrder.setFeePercent(BIDCECIMAL_ZERO);//目前没有手续费比例
            ...
            deliveryOrder.setFeePrice(BIDCECIMAL_ZERO.longValue());//目前没有设置手续费
            ...
            deliveryOrder.setExpireTime(DateUtil.afterNMin(new Date(),120));//订单超时时间(两小时)
            ...
            if(giftAccount.getAvailableUnitQuantity().compareTo(wantPickUpQuo) == -1){
                OperatorInfo operatorInfo = commodityService.queryOperaterInfo(deliveryOrder.getCommodityContractId());
                RespResult operator_respResult =  tradeAccountService.baseFreeze(deliveryOrder.getCommodityContractId(),operatorInfo.getOperatorInfoId(), AccountType.OPERATOR,freeze,CommodityFlowType.PICK_UP,"提货申请（运营商部分）","提货申请（运营商部分）");
    ...
    private static boolean checkCondition(DeliveryOrder deliveryOrder){
        ...
        StringUtils.isEmpty(deliveryOrder.getAddressDetailInfo()) || deliveryOrder.getApplyCommodityStandardUnitQuantity().compareTo(BIDCECIMAL_ONE) == -1){
            return false;
        }
