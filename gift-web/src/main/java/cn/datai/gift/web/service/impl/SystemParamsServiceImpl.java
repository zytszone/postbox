...
public class SystemParamsServiceImpl implements SystemParamsService{
    ...
    public void querySystemParams() {
        List<SystemParam> list = systemParamMapperExt.selectByExample(new SystemParamExample());
        for (SystemParam systemParam : list){
            redisTemplate.boundHashOps(RedisConstants.SYSTEM_PARAMS).put(systemParam.getName(),systemParam.getValue());
        }
    }
