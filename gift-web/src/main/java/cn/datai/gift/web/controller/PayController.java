...
public class PayController extends BaseController {
    ...
    public RespResult buyCommodity(@ModelAttribute("userInfoId") long userInfoId,
        ...
        try {
            PayInfo payInfo = buyCommodityService.buyCommodity(userInfoId,commodityId,quantity,quantityType);
            if(null == payInfo){
            ...
            logger.error("用户购买商品发生异常：用户Id:{},商品Id:{},购买数量：{},购买的类型：{},错误信息：{}",userInfoId,commodityId,quantity,quantityType,bize);
            if("2015".equals(bize.getErrorCode())){
                return new RespResult(RespCode.FAIL,"超出最大购买数量");
            }else if("2013".equals(bize.getErrorCode())){
                return new RespResult(RespCode.FAIL,"商品过期，不能购买");
            }else{
                return new RespResult(RespCode.FAIL,bize.getErrorMsg());
            }
        }catch (Exception e) {
            e.printStackTrace();
            logger.error("用户购买商品发生异常：用户Id:{},商品Id:{},购买数量：{},购买的类型：{},错误信息：{}",userInfoId,commodityId,quantity,quantityType,e);
            return new RespResult(RespCode.FAIL,"购买商品失败");
        }
    }
    ...
    public RespResult tradeOrderDetail(@ModelAttribute("userInfoId") Long userInfoId,@RequestParam("tradeOrderId") Long tradeOrderId) throws IOException {
        try {
            ...
            return new RespResult(RespCode.SUCCESS,new TradeOrderVo(tradeOrder,commodityVo));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @ResponseBody
    @Auth(needLogin = true,weixinJsAuth = true)
    public RespResult demoPay(@ModelAttribute("userLoginInfo")UserLoginInfo userLoginInfo,@RequestParam("tradeOrderId") String tradeOrderId,HttpServletRequest request) {
        try {
    ...
    public RespResult produceOrder(DeliveryOrder deliveryOrder,
        ...
        try {
            ...
            PayInfo payInfo = takeGoodsService.produceOrder(deliveryOrder);
            if(null == payInfo){
                return new RespResult(RespCode.SUCCESS, null);
            }
            HashMap<String, String> wxPayParams = new HashMap<>();//微信支付需要的参数
            ...
            logger.error("生成提货订单发生异常,提货订单信息：{},错误信息：{}", JSONObject.toJSONString(deliveryOrder),be);
            if("2014".equals(be.getErrorCode())){
                return new RespResult(RespCode.FAIL,"超出最大提货数量");
            }else if("2013".equals(be.getErrorCode())){
                return new RespResult(RespCode.FAIL,"超过有效的提货日期");
            }else{
                return new RespResult(RespCode.FAIL,be.getErrorMsg());
            }
        }catch (Exception e) {
            e.printStackTrace();
            logger.error("生成提货订单发生异常,提货订单信息：{},错误信息：{}", JSONObject.toJSONString(deliveryOrder),e);
            return new RespResult(RespCode.FAIL,"提货发生异常");
        }
    }
