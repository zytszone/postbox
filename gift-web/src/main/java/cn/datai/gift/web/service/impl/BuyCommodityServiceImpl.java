...
public class BuyCommodityServiceImpl implements BuyCommodityService {
    ...
    public PayInfo buyCommodity(long userInfoId, long commodityId, BigDecimal quantity, BigDecimal quantityType) throws IOException {
        ...
        if(amount <= 0){
            this.updateBuyCommodityInfoNoPay(tradeOrder);
            return null;
        }else{
            PayInfo payInfo = payService.createPayOrder(userInfoId, 1L, amount, PayBizType.BUY_COMMODITY, tradeOrder.getTradeOrderId().toString(), null, "test");
            //更新收支信息
    ...
    public void paidBuyCommodity(Long payInfoId) throws WxException {
    ...
    public void successBuyCommodity(TradeOrder tradeOrder) {
        ...
        tradeOrderMapperExt.updateByPrimaryKeySelective(tradeOrder);
        this.tradeAccountService.baseUnfreeze(tradeOrder.getCommodityId(),tradeOrder.getOpponentAccountId(), AccountType.OPERATOR,tradeOrder.getAccountId(),AccountType.USER,tradeOrder.getCommodityUnitQuantity(), CommodityFlowType.BUY,"买商品付款后的解冻","买商品付款后的解冻");
    ...
    @Transactional
    @Override
    public void cancelBuyCommodityInfo(Long tradeOrderId,Long userInfoId) {
        ...
        if(tradeOrder.getStatus().equals(TradeOrderStatus.NOPAY.getPersistKey())){
            tradeAccountService.baseUnfreeze(tradeOrder.getCommodityId(),tradeOrder.getOpponentAccountId(),AccountType.OPERATOR,tradeOrder.getOpponentAccountId(),AccountType.OPERATOR,tradeOrder.getCommodityUnitQuantity(),CommodityFlowType.BUY_CANCEL,"购买退还商品","购买退还商品");
    ...
    @Transactional
    @Override
    public void cancelExpireBuyCommodityInfo(Long tradeOrderId) {
        TradeOrder tradeOrder = tradeOrderMapperExt.selectByPrimaryKeyAndLock(tradeOrderId);
        if(tradeOrder.getStatus().equals(TradeOrderStatus.NOPAY.getPersistKey())){
            tradeAccountService.baseUnfreeze(tradeOrder.getCommodityId(),tradeOrder.getOpponentAccountId(),AccountType.OPERATOR,tradeOrder.getOpponentAccountId(),AccountType.OPERATOR,tradeOrder.getCommodityUnitQuantity(),CommodityFlowType.BUY_CANCEL,"购买退还商品","购买退还商品");
            tradeOrder.setStatus(TradeOrderStatus.CANCELLED.getPersistKey());
    ...
    @Transactional
    @Override
    public void updateBuyCommodityInfoNoPay(TradeOrder tradeOrder) {
        tradeOrder.setStatus(TradeOrderStatus.SUCCESS.getPersistKey());
        tradeOrderMapperExt.updateByPrimaryKeySelective(tradeOrder);
        this.tradeAccountService.baseUnfreeze(tradeOrder.getCommodityId(),tradeOrder.getOpponentAccountId(), AccountType.OPERATOR,tradeOrder.getAccountId(),AccountType.USER,tradeOrder.getCommodityUnitQuantity(),CommodityFlowType.BUY,"买商品付款后的解冻","买商品付款后的解冻");
