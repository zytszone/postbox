...
public class TradeServiceImpl implements TradeService {
    ...
    public RespResult order(long userInfoId, PayBizType payBizType, long commodityId, BigDecimal commodityQuantity, String[] couponDetailIds, BigDecimal quantityType, String remark) throws IOException {
        ...
        if(quantityType.compareTo(new BigDecimal(1)) == 0){
            //最小商品单位
        ...
        if(new BigDecimal(2).compareTo(quantityType)==0){
        //按瓶(饼)提取
            quantity = commodityQuantity.multiply(commodityContract.getStandardUnitMultiple());
        }
        if(new BigDecimal(1).compareTo(quantityType)==0){
        //按照毫升（克）提取
            quantity = commodityQuantity;
        }
        tradeOrder.setCommodityUnitQuantity(quantity);//买卖的数量commodityContract
        tradeOrder.setCommodityUnitPrice(commodityContract.getUnitPrice());//商品单位价格
        tradeOrder.setFeePercent(new BigDecimal(0));//目前手续费比例默认为0
        tradeOrder.setFeePrice(0L);//目前手续费默认为0
        BigDecimal totalPrice = quantity.multiply(new BigDecimal(commodityContract.getUnitPrice()));
        ...
        tradeOrder.setExpireTime(DateUtil.afterNMin(new Date(),120));//2小时订单超时
        tradeOrder.setStatus(TradeOrderStatus.NOPAY.getPersistKey());//订单状态
        tradeOrder.setRemark(payBizType.getName());//备注

        RespResult respResult = tradeAccountService.baseFreeze(commodityId,commodityContract.getOperatorInfoId(),AccountType.OPERATOR,quantity, CommodityFlowType.BUY,"用户购买商品","用户购买商品");
        ...
        userBizBrief.setOpCommodityUnitQuantity(quantity);//操作商品数量
