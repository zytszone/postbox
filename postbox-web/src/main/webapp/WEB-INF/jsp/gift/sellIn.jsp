...
<div id="sellIn">
    ...
    <div class="search-box">
        <span class="active" id="0">全部</span>
        <template v-for="(item,index) in typeList">
            <span v-bind:id="item.commodityTypeId">{{item.name}}</span>
        </template>
    </div>
    <div id="wrapper">
        <div class="scroll" id="scrollList">
            <ul class="search-list">
                <li class="gift-info" v-for="item in list">
                    <a v-bind:href="detailUrl+item.commodityContractId">
                        <div class="product-img">
                            <img v-bind:src="item.img">
                        </div>
                        <div>
                            <p class="gift-name">[{{item.typeName}}]{{item.name}}</p>
                            <p class="gift-detail"><span class="gift-money">￥{{item.money}}/{{item.standardUnitName}}</span><span class="desc">{{item.standardUnitMultiple}}{{item.unit}}/{{item.standardUnitName}}</span></p>
                        </div>
                    </a>
                </li>
            </ul>
...
<script type="text/javascript" src="${staticPath}/static/js/views/sellIn.js"></script>
</body>
</html>