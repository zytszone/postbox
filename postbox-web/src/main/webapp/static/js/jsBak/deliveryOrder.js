...
var deliveryRequest = new Vue({
    ...
    methods:{
                ...
                success: function (rsp) {
                    if(rsp.code == 0) {
                        ...
                        that.product = {
                            ...
                            img:product.headImg?product.headImg:staticPath+'/static/images/product-img1.jpg',
                            name:product.commodityName,
                            number:standardUnitMultiple,
                            unit:product.unit,
                            money:(product.lastUnitPrice*Number(getQueryString('takeNumber'))*standardUnitMultiple/100).toFixed(2),
                            deduction:product.holdCommodityQuantity,
                            expressFee:0.00,
                            cash:(product.lastUnitPrice*(Number(getQueryString('takeNumber'))*standardUnitMultiple-product.holdCommodityQuantity)/100).toFixed(2)
                        }
                    }
                }
            });
        ...
        pay:function () {
            var that = this;
            $.ajax({
                url: basePath+'pickUp/produceOrder',
                type: "POST",
                data: {
